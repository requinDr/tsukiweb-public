...
import { loadConfig, buildPaths, REPO_DIR, type Paths, type ToolConfig } from './orchestrator/pd-config.ts'
import {
  collectStatuses,
  createPrompt,
  printCheckDetails,
...
async function printStatus(statuses: StepStatus[], context: OrchestratorContext): Promise<void> {
  await printOrchestratorHeader({
    title: 'Tsukiweb Plus-Disc asset builder',
    subtitle: 'Augment the existing generated assets.',
    fields: [
